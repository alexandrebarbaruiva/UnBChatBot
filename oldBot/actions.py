    ...
    def on_chat_message(self, msg):
        ...
        try:
           guess = int(msg['text'])
            ...
            self.sender.sendMessage('Adivinha o número que eu estou pensando!')
            return
        ...
        if guess != self._answer:
            ...
            self.sender.sendMessage('Acertou!!')
            self.close()

    def on__idle(self, event):
        self.sender.sendMessage('Desistiu? A resposta era %d' % self._answer)
        self.close()
        time.sleep(1)
        self.sender.sendMessage('Tá aí?')
